...
const EditProudct = () => {
  const ProudctStatusArr = ['pending', 'interview', 'declined'];
  const ProudctTypeArr = ['full-time', 'part-time', 'internship'];
  const { Proudct } = useLoaderData();
  const { position, company, ProudctLocation, ProudctType, ProudctStatus } = Proudct;
  ...
  return (
    <Form method="post" className="form">
      ...
      < FormRowSelect name='ProudctType' labelText='jop type' defaultValue={ProudctType} list={ProudctStatusArr} />
      < FormRowSelect name='ProudctStatus' defaultValue={ProudctStatus} labelText='Proudct status' list={ProudctTypeArr} />
